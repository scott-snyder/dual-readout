...
DRcalib3D::DRcalib3D(const std::string& aName, ISvcLocator* aSvcLoc)
...
StatusCode DRcalib3D::execute(const EventContext&) const {
  ...
  for (unsigned int idx = 0; idx < hits2d->size(); idx++) {
    ...
    if (static_cast<float>(m_gateStart) != waveform.getTime()) {
      error() << "Gate start is not equal to the starting time of the waveform! waveform.getTime() = "
              << waveform.getTime() << endmsg;
      return StatusCode::FAILURE;
    }
    ...
    auto postprocTime =
        postprocTimes->create(); // create an object even if integral is 0 to match the order with other collections
    ...
    std::unique_ptr<TH1D> waveHist =
        std::make_unique<TH1D>("waveHist", "waveHist", m_nbins, m_gateStart, m_gateStart.value() + m_gateL.value());
    ...
    double toaProc = m_gateStart + m_gateL * waveProcessed->GetBinCenter(waveProcessed->FindFirstBinAbove(0.)) /
                                       static_cast<double>(m_nbins.value());
    ...
    if (integral > 0.) {
      for (int bin = 1; bin <= waveProcessed->GetNbinsX(); bin++) {
        ...
        double veloScaled =
            pSeg->IsCerenkov(cID) ? m_veloC->Interpolate(cen - toaProc) : m_veloS->Interpolate(cen - toaProc);
        veloScaled *= dd4hep::millimeter / dd4hep::nanosecond;
        double invVminusInvC =
            (veloScaled > 0.) ? 1. / veloScaled - 1. / dd4hep::c_light : std::numeric_limits<double>::max();
